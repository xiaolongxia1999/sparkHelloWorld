...
import TransferEntropy.spark
import TransferEntropy.spark.newUtils.{computeTwoSeriesTEsStringResultGaussian,findBestArmaP}
import TransferEntropy.spark.newUtils.findBestArmaP
/**
...
/**
  ...
  */
//引入显著性检验，统计概率
object computeTEWithSparkContinuousStandaloneArrayByRDD3 {
  def main(args: Array[String]): Unit = {
    ...
    val rdd = sparkContext.textFile((inputFilePath))

    val rdd1 = rdd.map(x=>x.replaceFirst(",","@"))
    val lines_cartesian = rdd1.cartesian(rdd1).repartition(24)


  //时序定阶————注意Vector是scala自带的类型，Vector则是spark中自定义的类（非类型),且vector类所在包，有静态类Vectors，勿混淆
//    val map_ts_k = rdd1.map( x=>
//      ( x.split("@")(0),findBestArmaP(Vectors.dense(x.split("@")(1).map(_.toDouble).toArray),maxP,maxD,maxQ   )         )      //findBestArmaP( DenseVector,maxP,maxD,maxQ )
//    )
...
//              srcSeries和destSeries是Array[Double]类型， 作为传递熵值计算的输入，两个 double[]类型的序列
            ...
            .map( x => (x._1+"->"+x._3 , computeTwoSeriesTEsStringResultGaussian(x._2,x._4 ,3,k_max,k_tau,l_max,l_tau,delay_max) ) )
