...
/**
  ...
  */
//引入显著性检验，统计概率
object computeTEWithSparkContinuousStandaloneArrayByRDD1 {
...
//    lines_cartesian.cache()
    ...
    val   rowRDD = lines_cartesian
      .map(
        x=>( x._1.split("@")(0)+"->"+x._2.split("@")(0), utils.computeTwoSeriesTEsStringResult( x._1.split("@")(1).split(",").map(_.toDouble).toArray,x._2.split("@")(1).split(",")
